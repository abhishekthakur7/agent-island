...
/// Settings is intentionally a conventional window-hosted view. It is the
/// only normal activation path; withdrawing the overlay never withdraws this
/// durable configuration and monitoring surface.
struct AgentIslandSettingsView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            Form {
                Section("Island Overlay") {
                    Picker("Selected display", selection: $overlay.selectedDisplayID) {
                        ForEach(overlay.displays) { display in
                            Text(display.name).tag(Optional(display.id))
                        }
                    }
                    .onChange(of: overlay.selectedDisplayID) { _, selected in overlay.selectDisplay(id: selected) }
                    Text(overlay.displayStatus)
                        .font(.caption)
                        .foregroundStyle(overlay.selectedScreen == nil ? .orange : .secondary)
                    Toggle("Hide in full screen", isOn: $overlay.hideInFullscreen)
                        .onChange(of: overlay.hideInFullscreen) { _, _ in overlay.reconcilePresentation() }
                    Toggle("Suppress while a quiet scene is active", isOn: $overlay.quietSceneActive)
                        .onChange(of: overlay.quietSceneActive) { _, _ in overlay.reconcilePresentation() }
                    Toggle("Expand on hover", isOn: $overlay.hoverExpansionEnabled)
                }
            }
            .formStyle(.grouped)
            Divider()
            ContentView(presentation: presentation, fixtureController: fixtureController, horizon: horizon)
        }
        .frame(minWidth: 1_180, minHeight: 840)
    }
}
